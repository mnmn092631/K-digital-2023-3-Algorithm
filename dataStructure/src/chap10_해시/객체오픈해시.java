...
class OpenHash {
	...
	private Bucket searchNode(SimpleObject2 key) {
		...
		for (int i = 0; p.stat != Status.EMPTY && p.stat != Status.DELETED && i < size; i++) {
			if (p.stat == Status.OCCUPIED && p.getValue().sno.equals(key.sno))
				return p;
...
public class 객체오픈해시 {
	...
	public static void main(String[] args) {
		Menu menu; // 메뉴
		SimpleObject2 temp = new SimpleObject2();
		int result;
		...
		do {
			...
			case REMOVE:
				...
				if (result == 0) System.out.println(" 삭제 처리");
				else System.out.println(" 삭제 데이터가 없음");
				break;
